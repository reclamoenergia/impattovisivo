...
echo [3/4] Build unified_view_to_points one-folder
%PY% -m PyInstaller --noconfirm --clean unified_view_to_points.spec
if errorlevel 1 exit /b 1

echo [4/4] Check output executable
if not exist "dist\unified_view_to_points\unified_view_to_points.exe" (
    echo ERRORE: EXE non trovato in dist\unified_view_to_points\
    exit /b 1
)

echo Build completed: dist\unified_view_to_points\unified_view_to_points.exe
endlocal
